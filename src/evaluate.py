...
def evaluate_model(model, dataset):
    ...
    gt_dict, pr_dict, shot_to_end_frame_dict = dict(), dict(), dict()
    model.eval()
    for n, sample in enumerate(dataloader):
        ...
        pr_dict[sample["imdb_id"][0]] = sigmoid_out.detach().cpu()[0]
        shot_to_end_frame_dict[sample["imdb_id"][0]] = sample["shot_end_frame"].cpu()[0]
    scores = dict()
